...
describe("OpenClaw plaintext bearer guard", () => {
  ...
  it("keeps loopback HTTP silent", async () => {
    ...
    await handlers.get("before_prompt_build")?.({ prompt: "recall auth work" });
  ...
  it("warns once for non-loopback HTTP with a bearer secret", async () => {
    ...
    await handlers.get("before_prompt_build")?.({ prompt: "first" });
    await handlers.get("before_prompt_build")?.({ prompt: "second" });
