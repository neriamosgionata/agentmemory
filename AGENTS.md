...
- **Telemetry-only hooks** (`notification`, `post-tool-failure`, `post-tool-use`, `prompt-submit`, `stop`, `session-end`, `subagent-start`, `subagent-stop`, `task-completed`) write nothing to stdout. These MUST use fire-and-forget `fetch(..., { signal: AbortSignal.timeout(N) }).catch(() => {})` paired with `setTimeout(() => process.exit(0), 500).unref()`. The unawaited fetch dispatches the request; the unref'd `setTimeout` force-exits the process after the request has been flushed to the local daemon's socket buffer (~500ms is enough for single-request hooks — `stop` and default `session-end`; use 1500ms only for the multi-request path, i.e. `session-end` when `CLAUDE_MEMORY_BRIDGE=true`, so both fetches have time to start, especially when `AGENTMEMORY_URL` points to a remote daemon). Longer windows race the host's shutdown grace and surface as "Hook cancelled" (#991). Without the `setTimeout` Node keeps the event loop alive waiting for any in-flight fetch to settle, which means the hook still blocks Claude Code's next-prompt boundary for up to the AbortSignal duration — exactly the bug fire-and-forget is meant to fix. Hooks are executed by the host agent from the command strings `connect` writes, so their runtime is the host's PATH `node` by default; `AGENTMEMORY_HOOK_RUNTIME=bun` rewrites those commands to `bun` (only `node`/`bun` accepted, static `plugin/hooks/*.json` manifests unaffected).
