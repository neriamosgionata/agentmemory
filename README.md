...
agentmemory pins iii-engine v0.24.0 and won't attach to a different version (the worker can't speak another engine's protocol). Stop the other engine, then run `npx -y @agentmemory/agentmemory@latest`. It installs and runs the pinned v0.24.0 in `~/.agentmemory/bin`, leaving your own `iii` untouched.
...
Compatibility: this release targets `iii-sdk` 0.11.2 and pins iii-engine v0.24.0.
...
Install `iii-engine` manually. **agentmemory pins `iii-engine` to `v0.24.0`**, the newest engine that runs the current direct-registration worker model. Engine `v0.23.0` switches to the `iii compose` worker-compose.yaml lifecycle that agentmemory has not been refactored onto yet. Override with `AGENTMEMORY_III_VERSION=<version>` if you've migrated to the Compose model manually.
...
Or use Docker (the bundled `docker-compose.yml` pulls `iiidev/iii:0.24.0`). Full docs: [iii.dev/docs](https://iii.dev/docs).

### Windows

agentmemory runs on Windows 10/11, but the Node.js package alone isn't enough; you also need the pinned iii-engine v0.24.0 runtime as a background process. The CLI does not auto-extract the Windows ZIP, so native Windows users must install `iii.exe` manually, use WSL2, or choose Docker Desktop.
...
#    (we pin to v0.24.0 — engine v0.23+ requires the iii compose model,
#     which agentmemory has not been refactored onto yet)
...
**Prerequisites:** Node.js >= 20 with npm/npx; [iii-engine](https://iii.dev/docs) v0.24.0 or Docker. The macOS/Linux automatic engine install also requires `curl`, a POSIX `sh`, and `tar`; native Windows uses the manual pinned `iii.exe`, WSL2, or Docker Desktop.
