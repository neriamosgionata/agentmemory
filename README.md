...
Compatibility: this release targets `iii-sdk` 0.11.2 and pins iii-engine v0.22.1.
...
Install `iii-engine` manually. **agentmemory pins `iii-engine` to `v0.22.1`**, the newest engine that runs the current direct-registration worker model. Engine `v0.23.0` switches to the `iii compose` worker-compose.yaml lifecycle that agentmemory has not been refactored onto yet. Override with `AGENTMEMORY_III_VERSION=<version>` if you've migrated to the Compose model manually.
...
#    (we pin to v0.22.1 — engine v0.23+ requires the iii compose model,
