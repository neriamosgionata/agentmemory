import type { ISdk, ApiRequest } from "../iii.js";
...
// Slot tools stay in the advertised surface (docs/manifests count them), but
// calling one while AGENTMEMORY_SLOTS is off used to surface the engine's
// opaque "Internal error" 500. Return the same actionable shape REST does. #888
function slotsDisabledResult(): { status_code: number; body: Record<string, string> } {
  return {
    status_code: 400,
    body: {
      error: "Memory slots not enabled",
      flag: "AGENTMEMORY_SLOTS",
      hint: "Set AGENTMEMORY_SLOTS=true (in ~/.agentmemory/.env or the shell) and restart.",
    },
  };
}
        ...
        switch (name) {
          ...
          case "memory_patterns": {
            const result = await sdk.trigger({ function_id: "mem::patterns", payload: {
              ...
              ...(typeof args.maxSessions === "number" && Number.isFinite(args.maxSessions) && { maxSessions: args.maxSessions }),
              ...(typeof args.sinceDays === "number" && Number.isFinite(args.sinceDays) && { sinceDays: args.sinceDays }),
            } });
            return {
              status_code: 200,
              body: {
          ...
          case "memory_sessions": {
            // Bounded, projected list (#1360): the old handler returned every
            // session row pretty-printed with its full summary. The full row
            // is one memory_recall away.
            try {
              if (
          ...
          case "memory_vision_search": {
            ...
            const topK = Math.max(
              ...
              Math.min(
                50,
                asNumber(args.topK, 10) ?? asNumber(args.limit, 10) ?? 10,
              ),
            );
          ...
          case "memory_action_create": {
            ...
            if (typeof args.createdBy === "string" && args.createdBy.trim()) {
              actionPayload.createdBy = args.createdBy.trim();
            }
          ...
          case "memory_slot_list": {
            if (!isSlotsEnabled()) return slotsDisabledResult();
