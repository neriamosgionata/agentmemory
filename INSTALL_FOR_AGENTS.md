...
Expect: iii-engine v0.24.0 starts, the agentmemory worker registers with iii, and the ready panel lists REST, viewer, streams, and engine addresses. First boot can take longer while the engine binary is downloaded.
...
- Native Windows: the CLI does not auto-extract the ZIP. Install the pinned v0.24.0 `iii.exe` manually, use WSL2, or run with Docker Desktop and `AGENTMEMORY_USE_DOCKER=1`. Automated `connect` supports only `copilot-cli`; configure other native agents manually.
...
- iii-engine v0.24.0 running, the selected platform data directory, and all four ports accounted for
